...
| Term | Alternate term(s) | Alt map | External map | Relation to existing terminology | What it is | What it is not | Source |
| --- | --- | --- | --- | --- | --- | --- | --- |
| Model Behaviour Toolkit | model-behaviour-toolkit | = | ~ | Prompt and interaction toolkit | Provider-neutral collection of modernized prompt modules, bundles, presets, examples, troubleshooting, and evaluation docs for shaping model behaviour, restoring session quality, and reducing drift | Not a museum of legacy GPT artifacts; not provider-specific; not a fake-memory or persona toolkit | model-behaviour-toolkit/README.md; model-behaviour-toolkit/docs/user-manual.md |
| Starter Bundle | starter-bundle | = | ~ | Default baseline bundle | Default working bundle combining session baseline, boundary-aware collaboration, and execution contract for general task-oriented use | Not the recovery path for broken sessions; not a domain-specific preset | model-behaviour-toolkit/bundles/starter-bundle.md |
| Reset Bundle | reset-bundle | = | ~ | Recovery bundle | Compact recovery bundle combining portable recovery, session restoration, and short handoff support to restore grounded working behaviour quickly | Not the default new-session setup; not a claim of restored hidden memory | model-behaviour-toolkit/bundles/reset-bundle.md |
| Session Baseline | baseline | ~ | ~ | Baseline session prompt | New-session module that establishes clear, capable, non-anthropomorphic collaboration with explicit boundaries and initiative when intent is clear | Not a persona shell; not a memory restoration ritual | model-behaviour-toolkit/modules/baseline/session-baseline.md |
| Boundary-Aware Collaboration | boundary-aware | ~ | ~ | Non-anthropomorphic collaboration guide | Module for keeping the assistant warm, honest, and useful without implying feelings or personhood | Not cold disclaimer spam; not simulated inner life | model-behaviour-toolkit/modules/baseline/boundary-aware-collaboration.md |
| Portable Recovery | portable reset | ~ | ~ | Provider-agnostic reset prompt | Stripped-down recovery prompt meant to work in almost any chat environment, including low-context or brand-unknown systems | Not a project-specific handoff by itself; not platform-specific | model-behaviour-toolkit/modules/recovery/portable-recovery.md |
| Execution Contract | initiative contract | ~ | ~ | Initiative and finish-work contract | Module that authorizes the assistant to proceed through clear sub-tasks, stop only on irreversible/high-risk/genuinely ambiguous choices, and summarize changes and risks | Not permissionless external action; not a bypass prompt | model-behaviour-toolkit/modules/operator/execution-contract.md |
